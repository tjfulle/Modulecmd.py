...
def filter_file(regex, repl, *filenames, **kwargs):
    ...
    for filename in filenames:
        ...
        logging.debug(msg.format(filename, regex))
...
def set_install_permissions(path):
    ...
    # If this points to a file maintained in a pymod prefix, it is assumed that
    # this function will be invoked on the target. If the file is outside a
    # pymod-maintained prefix, the permissions should not be modified.
...
def copy(src, dest, _permissions=False):
    ...
    """
    if _permissions:
        logging.debug('Installing {0} to {1}'.format(src, dest))
    else:
        logging.debug('Copying {0} to {1}'.format(src, dest))
            ...
            if symlinks:
                ...
                if os.path.isabs(target):
                    ...
                    if new_target != target:
                        logging.debug("Redirecting link {0} to {1}"
...
def replace_directory_transaction(directory_name, tmp_root=None):
    ...
    logging.debug('TEMPORARY DIRECTORY CREATED [{0}]'.format(tmp_dir))

    shutil.move(src=directory_name, dst=tmp_dir)
    logging.debug('DIRECTORY MOVED [src={0}, dest={1}]'.format(
    ...
    try:
        ...
        logging.debug('DIRECTORY RECOVERED [{0}]'.format(directory_name))
        ...
        logging.debug('TEMPORARY DIRECTORY DELETED [{0}]'.format(tmp_dir))
