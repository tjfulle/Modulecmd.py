...
def test_mc_execmodule_load_first(tmpdir, mock_modulepath):
    mock_modulepath(tmpdir.strpath)
    load_first = pymod.callback.get_callback("load_first")
...
def test_mc_execmodule_modulepath_ops(tmpdir, mock_modulepath):
    ...
    f1 = x.join("1.py").write(
        "append_path({0!r}, {1!r})".format(pymod.names.modulepath, one.strpath)
    )
    f2 = x.join("2.py").write(
        "prepend_path({0!r}, {1!r})".format(pymod.names.modulepath, two.strpath)
    )
