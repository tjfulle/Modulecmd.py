...
from modulecmd.command.common.parse_module_options import IllFormedModuleOptionError
from modulecmd.command.common.parse_module_options import parse_item_for_module_option
...
def test_command_parse_module_options_good_2():
    args = ["baz", "foo=bar", "+spam", "-eggs", "foo", "spam=4", "eggs=False", "-baz"]
    argv = parse_module_options(args)
    assert len(argv) == 2
