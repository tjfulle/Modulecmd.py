...
def test_config_default():
    config = modulecmd.config.factory()
...
def test_confing_user():
    basename = modulecmd.names.config_file_basename
    user_config_file = os.path.join(modulecmd.paths.user_config_path, basename)
    user_config = """\
config:
  debug: true
  verbose: true
...
def test_confing_admin():
    basename = modulecmd.names.config_file_basename
    admin_config_file = os.path.join(modulecmd.paths.etc_path, basename)
    admin_config = """\
config:
  debug: true
  verbose: true
...
config:
  ...
  serialize_chunk_size: {serialize_chunk_size}""".format(
        **config_dict["user"]
    )
    with open(user_config_file, "w") as fh:
        fh.write(user_config)
